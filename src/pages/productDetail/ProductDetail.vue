<template>
  <Breadcrum v-bind:listRoute="[{ name: 'Product detail' }, { name: productDetail?.name }]" />
  ...
  <div class="container pt-4 pb-6 grid lg:grid-cols-2 gap-6">
    ...
    <div>
      ...
      <div class="grid grid-cols-5 gap-4 mt-4">
        <div v-for="image in productDetail?.imageList" :key="image">
          <img
            :src="image"
            :class="'single-img w-full cursor-pointer ' + [targetImage === image && 'border border-primary']"
    ...
    <div>
      ...
      <div class="flex items-center mb-4">
        ...
        <div class="text-xs text-gray-500 ml-3">({{ productDetail.reviewCount }} Reviews)</div>
      </div>
      <div class="space-y-2">
        <p class="text-gray-800 font-semibold space-x-2">
          <span>Availability: </span>
          <span :class="[productDetail.inStock ? 'text-green-600' : 'text-red-600']">{{
        ...
        <p class="space-x-2">
          ...
          <span class="text-gray-600">{{ productDetail.category }}</span>
        </p>
        <p class="space-x-2">
      ...
      <div class="mt-4">
        ...
        <div class="flex items-center gap-2">
          <!-- single size -->
          <div v-for="size in sizeList" :key="size" class="size-selector">
            <input type="radio" name="size" class="hidden" :id="'size-' + size" :value="size" v-model="targetSize" />
            <label
              :for="'size-' + size"
  ...
  <div class="container pb-16">
    ...
    <div class="grid lg:grid-cols-4 sm:grid-cols-2 gap-6">
      <Card v-for="product in relativePrds" :key="product.id" v-bind="product" />
    </div>
    <!-- product wrapper end -->
  </div>
...
const productDetail = reactive<CardProps>(allProduct.find((product) => product.id === route.params.id) || defaultPrd)
...
const discountedPrice = computed(() => {
  return Number(productDetail?.price - (productDetail?.price * productDetail?.discount) / 100).toFixed(2)
})
...
const colorList = computed(() =>
  productDetail.sizes
    .map((size) => size.items.map((item) => (size.name === targetSize.value ? item.color : undefined)))
    .flat()
    .filter((item) => !!item)
)
...
const maxQuantity = computed(() => {
  const size = productDetail.sizes.find((size) => size.name === targetSize.value)
